package block.ciphers.modes.cbc;
...
/**
 * Class responsible for Simple In-line Initialising Vector CBC Mode Example.
 * 
 * Description:
 ...
 *   with CBC Mode, {some-padding} and Secret Key of 256-bit size, controlled by
 *   an Initialising Vector;
 */
public class SimpleInlineInitialisingVectorCBCModeExample {

	/**
	 * Main method. Performs an AES (Advanced Encryption Standard - Rijndael) Symmetric Encryption/De-cryption
	 * with CBC Mode, {some-padding}, Secret Key of 256-bit size, controlled by
	 * an Initialising Vector.
	 * 
	...
	public static void main(String[] args) throws Exception {
		...
		byte[] secretKeyBytes = new byte[] { 0x01, 0x23, 0x45, 0x67, 
										    (byte) 0x89, (byte) 0xab,
										    (byte) 0xcd, (byte) 0xef };
	    
        // The Initialising Vector bytes to be used
		byte[] initialisingVectorBytes = new byte[] { 0x07, 0x06, 0x05, 0x04,
				                                      0x03, 0x02, 0x01, 0x00 };
		...
		SecretKeySpec secretKeySpecifications = new SecretKeySpec(secretKeyBytes, "DES");
    	
		// The parameter specifications for the Initialising Vector
		IvParameterSpec initialisingVectorParameterSpecifications = new IvParameterSpec(new byte[8]);
        ...
        // Create the Cipher Text with the Initialising Vector bytes to be used,
        // in the beginning
        byte[] cipherText = new byte[cipher.getOutputSize( (initialisingVectorBytes.length + inputDataBytes.length ))];
        
        int cipherTextLength = cipher.update(initialisingVectorBytes, 0, initialisingVectorBytes.length, cipherText, 0);
        cipherTextLength += cipher.update(inputDataBytes, 0, inputDataBytes.length, cipherText, cipherTextLength);
    	...
    	System.out.println("----------------------------------------------------------------------------------------------------------------------");
      	...
      	// The complete byte stream received (Cipher Text of Initialising Vector and Plain Text appended)
      	byte[] buffferReceivedBytes = new byte[cipher.getOutputSize(cipherTextLength)];
      	
      	int buffferReceivedBytesLength = cipher.update(cipherText, 0, cipherTextLength, buffferReceivedBytes, 0);
      	buffferReceivedBytesLength += cipher.doFinal(buffferReceivedBytes, buffferReceivedBytesLength);
      	
      	// The Plain Text of the bytes of the data input received through the communication channel,
      	// after removing the Initialising Vector bytes used, in the beginning
      	byte[] plainText = new byte[( buffferReceivedBytesLength - initialisingVectorBytes.length )];

      	System.arraycopy(buffferReceivedBytes, initialisingVectorBytes.length, plainText, 0, plainText.length);
