...
/**
 * Class responsible for Simple Nonce Initialising Vector CBC Mode Example.
 * 
 * Description:
 ...
 *   a Nonce as Initialising Vector;
 */
public class SimpleNonceInitialisingVectorCBCModeExample {
	...
	public static void main(String[] args) throws Exception {
		...
		// The message's number (Nonce)
        byte[] messageNumber = new byte[] { 0x00, 0x00, 0x00, 0x00,
        									0x00, 0x00, 0x00, 0x00 };
        ...
        IvParameterSpec initialisingVectorZeroFilledParameterSpecifications = 
        ...
        // Generate an Initialising Vector for Encryption, based in the message number (Nonce)
        cipher.init(Cipher.ENCRYPT_MODE, secretKeySpecifications, initialisingVectorZeroFilledParameterSpecifications);
        IvParameterSpec initialisingVectorEncryptionParameterSpecifications = new IvParameterSpec(cipher.doFinal(messageNumber), 0, 8);
        
        // The Encryption process and the ciphering of its respectively content
        cipher.init(Cipher.ENCRYPT_MODE, secretKeySpecifications, initialisingVectorEncryptionParameterSpecifications);

        // Create the Cipher Text with the Initialising Vector bytes to be used,
        // based in the message number (Nonce)
    	...
    	System.out.println("----------------------------------------------------------------------------------------------------------------------");
      	...
      	// Generate an Initialising Vector for De-cryption, based in the message number (Nonce)
        cipher.init(Cipher.ENCRYPT_MODE, secretKeySpecifications, initialisingVectorZeroFilledParameterSpecifications);
        IvParameterSpec initialisingVectorDecryptionParameterSpecifications = new IvParameterSpec (cipher.doFinal(messageNumber), 0, 8);
	
        // The De-cryption process and the de-ciphering of its respectively content
        cipher.init(Cipher.DECRYPT_MODE, secretKeySpecifications, initialisingVectorDecryptionParameterSpecifications);
        
        // The Plain Text of the bytes of the data input received through the communication channel,
        // after using the Initialising Vector for De-cryption, based in the message number (Nonce)
